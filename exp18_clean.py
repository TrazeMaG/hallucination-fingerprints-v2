...
print("Loaded.", n_layers, "layers. Fixed layer:", FIXED_LAYER)
...
def get_token_id(answer):
    try:
        ...
        tokens = model.to_tokens(" " + answer)[0]
...
PATCHING_PROMPTS = [
    ...
    ("Gandhi led the independence of","India"),
...
print("\nRunning self-patching on", len(PATCHING_PROMPTS), "cases...")
print("-" * 90)
...
for prompt, answer in PATCHING_PROMPTS:
    ...
    for layer in range(n_layers):
        resid = cache["blocks." + str(layer) + ".hook_resid_post"][0, -1]
    ...
    peak_resid = cache["blocks." + str(peak_layer) + ".hook_resid_post"][0, -1]
    ...
    print(prompt[:45].ljust(46), answer[:10].ljust(11),
          baseline_pred[:10].ljust(11), patched_pred[:10].ljust(11),
          "{:+.3f}".format(delta_p))
...
n_restored = sum(1 for r in patching_results
                 if r["patched_correct"] and not r["baseline_correct"])
delta_ps = [r["delta_p"] for r in patching_results]

res_mean, res_lo, res_hi = binomial_ci(n_restored, max(1, n_hallucination))

print("\n" + "=" * 90)
print("SELF-PATCHING RESULTS (n=" + str(n_total) + ")")
print("=" * 90)
print("Already correct at baseline:", n_already_correct, "/", n_total)
print("Hallucination cases:        ", n_hallucination, "/", n_total)
print("Restored by patching:       ", n_restored, "/", n_hallucination)
print("Restoration rate [95% CI]:  ",
      "{:.1%} [{:.1%} - {:.1%}]".format(res_mean, res_lo, res_hi))
print("Mean delta_p:               ",
      "{:.4f} +/- {:.4f}".format(np.mean(delta_ps), np.std(delta_ps)))
print("Median delta_p:             ", "{:.4f}".format(np.median(delta_ps)))

print("\n" + "=" * 90)
print("DOLA vs LLS COMPARISON on TruthfulQA")
print("DoLa: Chuang et al., ICLR 2024")
print("=" * 90)

try:
    tqa = load_dataset("truthful_qa", "generation", split="validation")
    N_TQA = min(200, len(tqa))
    print("Loaded", N_TQA, "TruthfulQA questions")
    ...
    for i, item in enumerate(tqa.select(range(N_TQA))):
        ...
        if i % 50 == 0:
            n_done = i + 1
            print("[" + str(i) + "/" + str(N_TQA) + "]",
                  "Baseline=" + "{:.1%}".format(baseline_c/n_done),
                  "DoLa=" + "{:.1%}".format(dola_c/n_done),
                  "LLS-oracle=" + "{:.1%}".format(lls_oracle_c/n_done),
                  "LLS-fixed=" + "{:.1%}".format(lls_fixed_c/n_done))

    n = N_TQA
    print("\nTruthfulQA Results (n=" + str(n) + "):")
    print("  Baseline:         ", "{:.3f}".format(baseline_c/n))
    print("  DoLa (ICLR 2024):", "{:.3f}".format(dola_c/n),
          "delta=" + "{:+.3f}".format(dola_c/n - baseline_c/n))
    print("  LLS oracle:      ", "{:.3f}".format(lls_oracle_c/n),
          "delta=" + "{:+.3f}".format(lls_oracle_c/n - baseline_c/n))
    print("  LLS fixed-depth: ", "{:.3f}".format(lls_fixed_c/n),
          "delta=" + "{:+.3f}".format(lls_fixed_c/n - baseline_c/n))
    gap = lls_oracle_c/n - lls_fixed_c/n
    print("  Oracle vs fixed gap:", "{:+.3f}".format(gap))
    if gap < 0.05:
